...
class GitSyncImplTest {
    ...
    fun setup() {
        ...
        Git.init().setDirectory(bareRepo).setBare(true).call().close()

        // Seed it with an initial commit
        val tempWork = Files.createTempDirectory("seed_work").toFile()
        val seedGit = Git.cloneRepository()
            .setURI(bareRepo.toURI().toString())
            .setDirectory(tempWork)
            .call()
        File(tempWork, "web/github.com/alice.gpg").also { it.parentFile.mkdirs() }
            .writeText("dummy encrypted content")
        seedGit.add().addFilepattern(".").call()
        seedGit.commit().setMessage("initial").setAuthor("test", "test@test.com").call()
        seedGit.push().call()
        seedGit.close()
    ...
    @Test
    fun `clone creates working copy from file-url bare repo`() = runTest {
    ...
    @Test
    fun `pull with no changes returns empty SyncResult`() = runTest {
        gitSync.clone(bareRepo.toURI().toString(), repoDir.toPath())
    ...
    @Test
    fun `pull fast-forwards and returns correct SyncResult`() = runTest {
        gitSync.clone(bareRepo.toURI().toString(), repoDir.toPath())

        // Push a new entry to bare repo
        val tempWork = Files.createTempDirectory("push_work").toFile()
        val pushGit = Git.cloneRepository()
            .setURI(bareRepo.toURI().toString())
            .setDirectory(tempWork)
            .call()
        File(tempWork, "web/example.com/bob.gpg").also { it.parentFile.mkdirs() }
            .writeText("dummy")
        pushGit.add().addFilepattern(".").call()
        pushGit.commit().setMessage("add bob").setAuthor("test", "test@test.com").call()
        pushGit.push().call()
        pushGit.close()
        tempWork.deleteRecursively()
    ...
    @Test
    fun `pull returns NotFastForward when remote diverged`() = runTest {
        gitSync.clone(bareRepo.toURI().toString(), repoDir.toPath())
        ...
        localGit.commit().setMessage("local diverge").setAuthor("test", "test@test.com").call()
        localGit.close()
        ...
        pushGit.add().addFilepattern(".").call()
        pushGit.commit().setMessage("remote diverge").setAuthor("test", "test@test.com").call()
        pushGit.push().call()
        pushGit.close()
        tempWork.deleteRecursively()
    ...
    @Test
    fun `syncStatus reflects last pull timestamp`() = runTest {
        gitSync.clone(bareRepo.toURI().toString(), repoDir.toPath())
        gitSync.pull()
    ...
    @Test
    fun `syncStatus returns remoteReachable=false for invalid path`() = runTest {
        val status = gitSync.syncStatus()
