...
## 1. Project Scaffold
- [ ] Create Android project (Kotlin, minSdk 26, targetSdk 36)
...
- [ ] Set up `@HiltAndroidApp` application class
- [ ] Set up empty `NavHost` with placeholder destinations
- [ ] Confirm project builds and empty app launches
...
- [ ] Define `KeyManagement` interface
- [ ] Implement Keystore wrapping key generation (AES-256-GCM)
- [ ] Implement encrypt/decrypt blob helpers (app-private storage)
- [ ] Implement `clearAllKeys()` — wipes blobs + Keystore entries
...
- [ ] Implement `importGpgKey(armoredKey, passphrase?)`
        — parse armored key, decrypt if passphrase given, re-encrypt under wrapping key
- [ ] Implement `getGpgKey(): GpgPrivateKey` (unwrap blob, return in-memory key)
- [ ] Reject malformed armored key with `KeyImportError`
...
- [ ] Implement `generateSshKey(): PublicKey` — Ed25519, store encrypted blob
- [ ] Implement `getSshKey(): SshPrivateKey` (unwrap blob, return in-memory key)
...
- [ ] Wire `BiometricPrompt` into `getGpgKey()` and `getSshKey()`
- [ ] Implement session inactivity timer (default 5 min, configurable via DataStore)
- [ ] Degrade gracefully when biometric not enrolled (fall back to device PIN)
...
## 6. Git Sync Module
- [ ] Define `GitSync` interface + `SyncResult`, `SyncStatus`, `SyncError` sealed types
- [ ] Implement `clone(remoteUrl, localPath)` via JGit + MINA sshd
- [ ] Implement `pull()` — fast-forward only (`--ff-only`), return `SyncResult`
- [ ] Implement `syncStatus()` — last sync time, remote reachable check
- [ ] Hilt `@Singleton` binding, all ops on `Dispatchers.IO`
- [ ] Add test fixtures: local bare git repo with sample `.gpg` files

**Tests:**
- [ ] clone() creates working copy from `file://` bare repo
- [ ] pull() returns correct newEntries / removedEntries
- [ ] pull() with no changes returns empty SyncResult
- [ ] pull() returns `SyncError.NotFastForward` when remote diverged
- [ ] syncStatus() reflects last pull timestamp
- [ ] syncStatus() returns `RemoteUnreachable` for invalid path
...
## 7. Pass Store Module
- [ ] Define `PassStore` interface + `PassEntry` data class
- [ ] Implement `buildIndex()` — walk git working copy, parse all `.gpg` paths
- [ ] Implement path parsing: `web/github.com/alice.gpg` → domain + username
- [ ] Implement `resolve(domain)` — exact → subdomain → fuzzy (Levenshtein)
- [ ] Implement `resolve(packageName)` — strip prefix, map to domain
- [ ] Implement `search(query)` — case-insensitive ranked search
- [ ] Expose index as `StateFlow<List<PassEntry>>`
- [ ] Hilt `@Singleton` binding

**Tests:**
- [ ] buildIndex() parses `web/github.com/alice.gpg` → domain=github.com, username=alice
- [ ] buildIndex() handles 1-level path → domain=null
- [ ] buildIndex() ignores non-.gpg files
- [ ] resolve(domain) exact match returns entry first
- [ ] resolve(domain) subdomain: github.com resolves gist.github.com query
- [ ] resolve(domain) fuzzy: "githubb.com" returns github.com entry
- [ ] resolve(domain) returns empty list when no candidates
- [ ] resolve(packageName) maps com.github.android → github.com entry
- [ ] search() is case-insensitive
- [ ] search() ranks closer matches higher
...
## 8. Decryption Module
- [ ] Define `Decryption` interface + `Credentials`, `AutofillCredentials` types
        (`password: CharArray`, `notes: String`)
- [ ] Add test resources: throwaway GPG keypair + pre-encrypted fixture `.gpg` files
        (single-line, multi-line, wrong-key)
- [ ] Implement `decrypt(entry)` via PGPainless — calls `KeyManagement.getGpgKey()`
- [ ] Implement `decryptForAutofill(entry)` — password only, no notes
- [ ] Implement `CharArray.zero()` extension, call after use
- [ ] Hilt `@Singleton` binding, ops on `Dispatchers.IO`

**Tests:**
- [ ] decrypt() returns password from line 1 (single-line fixture)
- [ ] decrypt() returns password + notes (multi-line fixture)
- [ ] decrypt() throws DecryptionError for wrong-key fixture
- [ ] decrypt() throws DecryptionError for corrupted file
- [ ] decryptForAutofill() returns only password
- [ ] CharArray is all-zero after explicit clear call
